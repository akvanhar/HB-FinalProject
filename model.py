...
class Friendship(db.Model):
    ...
    friendship_id = db.Column(db.Integer,
                              autoincrement=True,
                              primary_key=True)
    ...
    user = db.relationship("User",
                           ...
                           backref=db.backref("friendships",
                                              order_by=friendship_id))

    def __repr__(self):
        # A helpful representation of the relationship

        return ("Friendship between admin_id: \
                %s and friend_id: %s>") % (self.admin_id,
                                           self.friend_id)
...
class Food(db.Model):
    ...
    allergen_id = db.Column(db.Integer,
                            db.ForeignKey('allergens.allergen_id'),
                            nullable=False)
    ...
    def add_food(cls, title, texture, datemade, quantity,
                 freshfrozen, description, allergen_id,
                 user_id, location_id, phone_number, shared_with=""):
        ...
        food = cls(title=title, texture=texture,
                   ...
                   phone_number=phone_number, location_id=location_id,
    ...
    def share_food(self, food_id, user_id, friend_id):
        ...
        this_friendship = Friendship.query.filter_by(admin_id=user_id,
                                                     friend_id=friend_id
                                                     ).first()
...
class Message(db.Model):
    ...
    def __repr__(self):
        ...
        return ("Message id: %s sender_id:\
                %s, receiver_id: %s>") % (self.message_id,
                                          self.sender_id,
                                          self.receiver_id)
